...
* To uninstall the redmine_scheduler plugin (remove database):
  
  <tt>$ rake redmine:plugins:migrate NAME=redmine_scheduler VERSION=0</tt>

* Fresh database install (for redmine/plugins):

  <tt>$ rake db:drop </tt><br>
  <tt>$ rake db:create </tt><br>
  <tt>$ rake db:migrate </tt><br>
  <tt>$ rake redmine:plugins:migrate NAME=redmine_scheduler</tt><br>
